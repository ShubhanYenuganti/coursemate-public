...
The system SHALL extend the LLM JSON response schema to include an optional `clarifying_question` field (string or null). Both the agentic (`AGENTIC_JSON_FINAL_INSTRUCTION`) and non-agentic (`_SYNTHESIS_JSON_FINAL_INSTRUCTION`) prompts SHALL instruct the model to emit this field when it has made significant assumptions about the user's intent, and null otherwise.
...
- **THEN** the JSON response contains `clarifying_question` set to a single focused question string
...
- **THEN** the JSON response contains `clarifying_question: null`

#### Scenario: Parser extracts clarifying_question alongside existing fields
- **WHEN** `_parse_synthesis_json` processes a response containing `clarifying_question`
